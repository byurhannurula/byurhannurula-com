  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:border-border hover:bg-muted/50"
      >
        ...
        <kbd className="pointer-events-none hidden select-none rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium sm:inline">
          ⌘K
        </kbd>
      </button>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          <CommandGroup heading="Filter by Tag">
            {currentTag && (
              <CommandItem onSelect={clearTagFilter} className="text-muted-foreground">
                <X className="mr-2 h-4 w-4" />
                <span>Clear filter: {currentTag}</span>
              </CommandItem>
            )}
            {uniqueTags.map((tag) => (
              <CommandItem key={tag} value={`tag:${tag}`} onSelect={() => handleTagSelect(tag)}>
                <Tag className="mr-2 h-4 w-4" />
                <span className={currentTag === tag ? "font-medium text-primary" : ""}>{tag}</span>
                {currentTag === tag && <span className="ml-auto text-xs text-primary">active</span>}
              </CommandItem>
            ))}
          </CommandGroup>
